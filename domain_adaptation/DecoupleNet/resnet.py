...
path_to_resnet50 = "models/r50_1x_sk0.pth"
...
class model_bx_resnet(nn.Module):
...
class model_b_resnet(nn.Module):
    def __init__(self, layers, width_multiplier, sk_ratio):
        super().__init__()
        ops = [Stem(sk_ratio, width_multiplier)]
        channels_in = 64 * width_multiplier
...
def get_resnet_2(depth=50, width_multiplier=1, sk_ratio=0):  # sk_ratio=0.0625 is recommended
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = model_b_resnet(layers, width_multiplier, sk_ratio)
    resnet = torch.nn.Sequential(*(list(resnet.children())[:-1]))
    resnet_x = model_bx_resnet(layers, width_multiplier, sk_ratio)
    resnet_x2 = model_bx_resnet(layers, width_multiplier, sk_ratio)
    return resnet, resnet_x ,resnet_x2


def get_resnet(depth=50, width_multiplier=1, sk_ratio=0):  # sk_ratio=0.0625 is recommended
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    model_weights = torch.load(path_to_resnet50)
    
    resnet = ResNet(layers, width_multiplier, sk_ratio)
    resnet.load_state_dict(model_weights['resnet'], strict=False)
    resnet = torch.nn.Sequential(*(list(resnet.children())[:-1]))
    resnet_x = torch.nn.Sequential(*(list(resnet.children())[0][0:2]))
    resnet_x2 = torch.nn.Sequential(*(list(resnet.children())[0][0:2]))
    resnet =  torch.nn.Sequential(*(list(resnet.children())[0][2:]))
    return resnet, resnet_x ,resnet_x2
