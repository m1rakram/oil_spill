...
import constants
import domain_adaptation.ProCST.models as models
from domain_adaptation.ProCST.style_transfer import StyleTransferLoss
from models.segformer_utils.model import SegFormer, mit_b0, SegFormerHead
from domain_adaptation.ProCST.functions import imresize_torch, colorize_mask, reset_grads, \
...
def train(opt):
    opt.best_miou = 50
    semseg_cs = None
    opt.tb = SummaryWriter(comment="procst")
    ...
    while scale_num < opt.num_scales + 1:
        ...
        backbone = mit_b0()
        head = SegFormerHead(in_channels=[32, 64, 160, 256], num_classes=5, embed_dim=512)

        semseg_cs = SegFormer(backbone, head).cuda()
...
def train_single_scale(netDst, netGst, netDts, netGts, Gst: list, Gts: list, Dst: list, Dts: list,
    ...
    while keep_training:
        ...
        for batch_num, ((source_scales, source_label), (target_scales, _, _)) in enumerate(zip(opt.source_loaders[opt.curr_scale], opt.target_loaders[opt.curr_scale])):
            ...
            # S -> T:
            discriminator_losses = adversarial_discriminative_train(netDst, netGst, Gst, target_scales, source_scales, opt)
            for k,v in discriminator_losses.items():
                print('Scale%d/ST/Discriminator/%s' % (opt.curr_scale, k), v, discriminator_steps)

            # T -> S:
            discriminator_losses = adversarial_discriminative_train(netDts, netGts, Gts, source_scales, target_scales, opt)
            for k,v in discriminator_losses.items():
                print('Scale%d/TS/Discriminator/%s' % (opt.curr_scale, k), v, discriminator_steps)
...
def synthetic_semantic_segmentation_loss(input_pyramid, Gs, currG, semseg_net, input_label, opt):
    ...
    converted_image = currG(input_pyramid[-1], prev_converted_image, input_label if not opt.warmup else None)
...
def load_trained_networks(opt):
    ...
    if os.path.isfile(os.path.join(opt.continue_train_from_path, 'semseg_cs.pth')):
        ...
        if not hasattr(semseg_cs, 'ce_loss'):
            semseg_cs.ce_loss = nn.CrossEntropyLoss(ignore_index=-1)
    else:
        semseg_cs = None
...
def export_epoch_accuracy(opt, iou, miou, cm, epoch):
    ...
    print('================Model Accuracy Summery================')
    for i in range(constants.NUM_CLASSES_SEG):
        print('class accuracy: = %.2f%%' %  iou[i] * 100)
...
def calculte_validation_accuracy(semseg_net, target_val_loader, epoch_num, opt):
    semseg_net.eval()
    with torch.no_grad():
        running_metrics_val = runningScore(constants.NUM_CLASSES_SEG)
        cm = torch.zeros((constants.NUM_CLASSES_SEG, constants.NUM_CLASSES_SEG)).cuda()
        for val_batch_num, (target_images, target_labels) in enumerate(target_val_loader):
            ...
            with torch.no_grad():
                ...
                cm += compute_cm_batch_torch(pred_labels, target_labels, constants.IGNORE_INDEX, constants.NUM_CLASSES_SEG)
